...
import type { DataTableFilterField, ExtendedSortingState } from '@/types/data-table.type'
import {
...
export function useDataTable<TData>({
  ...
  const [page, setPage] = useQueryState(
    'page',
    parseAsInteger.withOptions(queryStateOptions).withDefault(PAGINATION.DEFAUT_PAGE_INDEX)
  )
  const [limit, setLimit] = useQueryState(
    'limit',
    parseAsInteger
      .withOptions(queryStateOptions)
      .withDefault(initialState?.pagination?.pageSize ?? PAGINATION.DEFAULT_PAGE_SIZE)
  )
