import type { ColumnType, Filter, FilterOperator } from '@/types/data-table.type'
...
export const formatSortQuery = (sortArr: { id: string; desc: boolean }[]) => {
  // Không truyền sort query trên URL
  if (sortArr.length === 0) return undefined
  const sortField = sortArr[0].id
  const isDescending = sortArr[0].desc
