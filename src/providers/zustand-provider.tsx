...
type Action = {
  ...
  updateOrderItem: (tableNumber: number, dishId: string, payload: { quantity?: number; note?: string }) => void
...
export const useAppStore = create<State & Action>((set) => ({
  ...
  addOrderItem: (tableNumber: number, newOrderItem: OrderItem) => {
    ...
    set((state) => {
      ...
      let prevItems = prevOrderItems.find((order) => order.tableNumber === tableNumber)?.items ?? []

      // loại bỏ đi items của bàn mà cần thêm mới các đơn hàng vô
      prevOrderItems = prevOrderItems.filter((order) => order.tableNumber !== tableNumber)
      // loại bỏ đi các món ăn đã có trong orderItems
      prevItems = prevItems.filter((item) => item.dish._id !== newOrderItem.dish._id)
      return { orderItems: [...prevOrderItems, { tableNumber, items: [...prevItems, newOrderItem] }] }
    })
  },
  removeOrderItem: (tableNumber: number, dishId: string) => {
    ...
    set((state) => {
      ...
      const filteredItems = prevItems.filter((item) => item.dish._id !== dishId)
      prevOrderItems[prevItemsIdx].items = filteredItems
      return { orderItems: [...prevOrderItems] }
    })
  },
  updateOrderItem: (tableNumber: number, dishId: string, payload: { quantity?: number; note?: string }) => {
    ...
    set((state) => {
      ...
      const prevItems = prevOrderItems[prevItemsIdx].items
      const targetItemIdx = prevItems.findIndex((order) => order.dish._id === dishId)
      if (targetItemIdx < 0) {
        return { orderItems: prevOrderItems }
      }
      const targetItem = prevItems[targetItemIdx]
      prevOrderItems[prevItemsIdx].items[targetItemIdx] = { ...targetItem, ...payload }
      return { orderItems: [...prevOrderItems] }
    })
  },
