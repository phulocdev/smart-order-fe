...
export async function refreshAccessToken(token: JWT): Promise<JWT> {
  ...
  try {
    if (token.account) {
      ...
      const response = await customerAuthApiRequest.refreshToken(
        token.refreshToken
      );
      const { data: newSession } = response;
      return newSession;
...
export const middleware: NextMiddleware = async (request: NextRequest) => {
  ...
  const isEmployeePage = employeeBasePaths.some((path) =>
    pathname.startsWith(path)
  );

  const isCustomer = !!token?.customer;
  const isCustomerPage = customerBasePaths.some((path) =>
    pathname.startsWith(path)
  );

  if (
    isAuthenticated &&
    authenticationBasePaths.some((path) => pathname.startsWith(path))
  ) {
