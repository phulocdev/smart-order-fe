...
import { SESSION_COOKIE, SESSION_SECURE, TOKEN_REFRESH_BUFFER_SECONDS } from '@/config/next-auth'
...
export async function refreshAccessToken(token: JWT): Promise<JWT> {
  ...
  try {
    if (token.account) {
      ...
      const response = await customerAuthApiRequest.refreshToken(token.refreshToken)
      const { data: newSession } = response
      return newSession
    }
    return token
