      ...
      <Popover>
        <PopoverTrigger asChild>
          <Button
            ...
            size={triggerSize}
            className={cn(
              'w-full justify-start gap-2 truncate text-left text-[13px] font-normal',
        ...
        <PopoverContent className={cn('w-auto p-0', className)} {...props}>
          <Calendar
            ...
            onSelect={(newDateRange) => {
              void setDateParams({
                from: newDateRange?.from ? format(newDateRange.from, 'yyyy-M-dd') : '',
                to: newDateRange?.to ? format(newDateRange.to, 'yyyy-M-dd') : ''
              })
            }}
            numberOfMonths={2}
          />
