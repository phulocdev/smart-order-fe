...
export default function AdvanceDateRange({ from, to, className }: Props) {
  const [calendarOpen, setCalendarOpen] = useState(false)
  if (from && to && isAfter(from, to)) {
    ;[to, from] = [from, to]
  }
  const initialFromDate = startOfDay(from)
  const initialToDate = endOfDay(to)
  ...
  const [dateParams, setDateParams] = useQueryStates(
    {
      from: parseAsString.withDefault(dateRange?.from?.toISOString() ?? ''),
      to: parseAsString.withDefault(dateRange?.to?.toISOString() ?? '')
    },
    {
      clearOnDefault: true,
      shallow: false
    ...
    <div className={className}>
      <Popover open={calendarOpen}>
        ...
        <PopoverContent className='w-auto p-0' align='start'>
          ...
          <div className='flex justify-end gap-x-4 px-8 pb-6'>
