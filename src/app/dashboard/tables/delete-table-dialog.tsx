...
import { useRemoveTableMutation } from "@/hooks/api/useTable";
import { handleApiError } from "@/lib/utils";

interface DeleteTableDialogProps
  extends React.ComponentPropsWithoutRef<typeof Dialog> {
  table?: Row<ITable>["original"];
  showTrigger?: boolean;
  onSuccess?: () => void;
}
...
export function DeleteTableDialog({
  ...
  async function onDelete() {
    try {
      if (!table) return;
      await removeTableMutation.mutateAsync(table._id);
      onSuccess?.();
    } catch (error) {
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="max-w-[90%] leading-6">
              Bạn có chắc chắn muốn xóa bàn số {table?.number}?
            </DialogTitle>
            <DialogDescription>
              Hành động này không thể được hoàn tác. Thao tác này sẽ xóa vĩnh
              viễn bàn ăn khỏi máy chủ.
            </DialogDescription>
          </DialogHeader>
  ...
  return (
    <Drawer {...props}>
      ...
      <DrawerContent>
        <DrawerHeader>
          <DrawerTitle>Bạn có chắc chắn muốn xóa bàn ăn không?</DrawerTitle>
          <DrawerDescription>
