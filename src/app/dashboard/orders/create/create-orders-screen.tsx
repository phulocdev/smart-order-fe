...
import { CreateCustomerBodyType } from '@/schemaValidations/customer.schema'
  ...
  return (
    <div className='grid grid-cols-12'>
      <div className='col-span-7'>
        <Tabs defaultValue='table' className='p-5'>
          <TabsList>
            <TabsTrigger value='table'>Bàn ăn</TabsTrigger>
            <TabsTrigger value='menu'>Thực đơn</TabsTrigger>
          </TabsList>
          <TabsContent value='table'>
            <div className='grid h-[80vh] cursor-pointer grid-cols-5 gap-4 overflow-y-scroll [&::-webkit-scrollbar]:w-0'>
              {tableData.map((table) => (
                <TableCard
                  key={table._id}
                  table={table}
          ...
          <TabsContent value='menu'>
            <div className='grid cursor-pointer grid-cols-5 items-start gap-4'>
              {dishData.map((dish) => (
      ...
      <div className='col-span-5'>
        <Tabs defaultValue={undefined} className='p-5'>
          <TabsList className={cn({ 'p-0': orderItemsHasOrders.length === 0 })}>
            ...
            {orderItemsHasOrders.map((orderItems, index) => {
              return (
                <TabsTrigger key={index} value={orderItems.tableNumber.toString()}>
          ...
          {orderItemsHasOrders.length === 0 && (
            <div className='flex h-[85vh] items-center justify-center'>Vui lòng chọn bàn để có thể đặt món !</div>
          )}
            ...
            return (
              <TabsContent value={order.tableNumber.toString()} key={order.tableNumber}>
                <div className='h-[80vh]'>
                  <Table divClassname='h-[66vh] custom-scrollbar rounded-sm overflow-y-scroll'>
                    ...
                    {/* <TableFooter className='sticky bottom-0 rounded-sm bg-gray-100'>
                      <TableRow>
                        <TableCell colSpan={3}>Tổng ({totalPortion} phần):</TableCell>
                  ...
                  <div className='mt-auto border-t border-t-gray-300 bg-white px-3 py-5'>
                    <div className='flex items-start justify-between pb-6'>
                      ...
                      <div>
                        ...
                        <span className='text-xl font-bold text-red-600'>{formatNumberToVnCurrency(totalPrice)}</span>
                        <div className='text-right text-sm font-semibold'>({totalPortion} món)</div>
                      </div>
                    </div>
                    <Button
                      onClick={() => {
