...
export default function OrdersCard() {
  ...
  const confirmOrders = async () => {
    try {
      const items: OrderItemDto[] = orderItems.map((order) => ({
        ...order,
        dish: order.dish._id,
      }));
  ...
  if (isDesktop) {
    return (
      <Card
        className={`sticky`}
        style={{ top: `calc(${HEADER_HEIGHT}px + 2rem)` }}
      >
        {!session?.customer && (
          <CardHeader>
            <CardTitle className="text-center font-normal">
              Vui lòng quét mã QR để có thể gọi món!
            </CardTitle>
          </CardHeader>
        )}
        ...
        <CardContent className="pr-0">
          {session?.customer && (
            <h3 className="py-3 text-center">
              Bàn số: {session?.customer?.tableNumber}
            </h3>
          )}
          <div className="custom-scrollbar flex max-h-[350px] flex-col gap-y-2 pr-4 pt-2 text-sm">
            {orderItems.map((orderItem, index) => (
              <div
                className="border-b border-b-gray-200 py-2.5 pb-3 last:border-b-0 last:pb-0"
                key={index}
              >
        ...
        <div className="border-t border-t-gray-300 bg-[#F0F0F0] px-4 py-5">
          <div className="flex justify-between">
            <div className="text-[15px] font-medium text-neutral-800">
              Tổng cộng ({orderItems.length} món):
            </div>
      ...
      <DrawerContent className="h-[60vh] rounded-none">
        ...
        <div className="custom-scrollbar overflow-y-scroll px-4 py-4">
          <div className="flex flex-col gap-y-2 pt-2 text-sm">
            {orderItems.map((orderItem, index) => (
              <div
                className="border-b border-b-gray-200 pb-7 last:border-b-0 last:pb-0"
                key={index}
              >
                <div className="flex items-start justify-between gap-x-7">
                  <div>
