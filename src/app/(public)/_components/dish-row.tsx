...
export default function DishRow({ dish, session }: Props) {
  ...
  if (isDesktop) {
    return (
      <Dialog
        open={
          !session?.customer || dish.status === DishStatus.Unavailable
            ? false
            : undefined
        }
      >
        ...
        >
          <div className="group flex cursor-pointer gap-x-4 border-b border-b-gray-200 py-5 last:border-b-0 last:pb-0">
            ...
            <div className="grow">
              ...
              <div
                className={cn("line-clamp-2 text-sm leading-5 text-gray-700", {
                  "py-1": !!dish.description,
                })}
              >
                {dish.description}
              </div>
              <div className="flex items-center gap-x-2 pt-4">
                <div className="font-heading font-semibold text-red-600">
                  {formatNumberToVnCurrency(dish.price)}
                </div>
                <div className="ml-auto inline-flex gap-x-3">
                  {dish.status === DishStatus.Unavailable && (
                    <Badge variant={"ghost"}>Hết hàng</Badge>
                  )}
                  <button
                    className={cn(
            ...
            <DialogTitle>
              <div className="flex items-start gap-x-3 border-b border-b-gray-200 py-4">
                <Image
                  ...
                  alt={dish.title}
                />
                <h3 className="font-heading font-medium text-third">
                  {dish.title}
                </h3>
              </div>
            </DialogTitle>
          </DialogHeader>
          <p className="line-clamp-4 font-normal leading-5 text-gray-600">
            {dish.description}
          </p>
      ...
      <DrawerContent className="h-screen">
        ...
        <div className="custom-scrollbar h-full overflow-y-scroll">
          ...
          <div className="mx-auto w-[75%]">
            <h2 className="mb-5 font-heading text-xl capitalize tracking-wider text-third">
              {dish.title}
            </h2>
